...
// 查看主板
$('#protocol').on('shown.bs.modal', function () {
    var board = $('#deviceType').val();
// $("#myModalLabel").text("Makeblock 主板：" + board);
var order = {
    "auriga": 0,
    "mcore": 1,
    "orion": 2,
...
function outputValue(hexStr) {
    ...
    if(hexStrArray[hexStrArray.length - 1] == "0a" && hexStrArray[hexStrArray.length - 2] == "0d") {
        if(hexStrArray[3] == "03") {
            ...
            // for(var i in hexStrArray) {
            //     x[i] = hexStrArray[i];
            // }
            // console.log(x);
            // 字符串
            $('#calValue').text(hexStrArray.join(" "));
        } else {
            // 4byte
